...
const NewOrders = () => {
  ...
  const fetchNewOrders = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/dashboard/new-orders?seller_id=${user?.seller_id}`);
      console.log("New orders data:", response.data);
      setOrders(response.data);
      setError(null);
    } catch (err) {
      console.error('Error fetching new orders:', err);
      setError('Failed to load new orders. Please try again.');
      setOrders([]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100">
          ...
          {orders.length === 0 ? (
            <div className="p-4 sm:p-6 text-center">
              <p className="text-gray-500 text-lg">No new orders at the moment</p>
            </div>
          ) : (
