import React from 'react';
import DeliveryLayout from '@/components/delivery/DeliveryLayout';
import { useDelivery } from '@/contexts/DeliveryContext';
import DeliveryCard from '@/components/delivery/DeliveryCard';
import DeliveryDetail from '@/components/delivery/DeliveryDetail';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';

const PendingDeliveries: React.FC = () => {
  const { tasks, setActiveTask, activeTask } = useDelivery();
  
  // Filter pending and in-transit deliveries
  const pendingDeliveries = tasks.filter(task => 
    ['pending', 'in-transit'].includes(task.status)
  );

  return (
    <DeliveryLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-2">Pending Deliveries</h1>
        <p className="text-muted-foreground">
          View and manage your pending delivery tasks
        </p>
      </div>
      
      <div className="max-w-3xl mx-auto">
        {activeTask ? (
          <DeliveryDetail 
            task={activeTask} 
            onBack={() => setActiveTask(null)} 
          />
        ) : (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-lg">Active Tasks</CardTitle>
            </CardHeader>
            <CardContent>
              {pendingDeliveries.length > 0 ? (
                <div className="space-y-4">
                  {pendingDeliveries.map(delivery => (
                    <DeliveryCard
                      key={delivery.id}
                      delivery={delivery}
                      onClick={() => setActiveTask(delivery)}
                    />
                  ))}
                </div>
              ) : (
                <div className="text-center py-6 text-muted-foreground">
                  <p>No pending deliveries available</p>
                </div>
              )}
            </CardContent>
          </Card>
        )}
      </div>
    </DeliveryLayout>
  );
};

export default PendingDeliveries; 